...
upper <- fun$a#+.01#seq(fun$a-.8, fun$a-0.2, by = 0.01)
  ...
  for(n in 1:length(upper)){
    ...
    for (k in 1:nrow(scenario)){
      ...
      for (j in 1:nsim){
        ...
        estimatedssIA[k,j] <- length(plac1st)+length(dur1st)+length(dur2st)
    ...
    summary_ORD <- summary_ORD %>% dplyr::mutate(
...
write.csv(summary_ORD, paste("3arm2stage_SI_corrected_ESSIA3_recrutrate", recrate, "_", Sys.Date(), 
                             ".csv", sep = ""),
row.names = F)    
