...
nm <- nc #number of patients needed on the medium arm

# number of patients in each arm at the interim analysis

interim <- c(n/2+recrate/length(durations)*c(durations[1]-durations[2:4]),n/2)

# ratio between number of patients on each treatment arm and on the control

allocatioratio <- interim/interim[4]
...
bounds_4arm2stageORDSF <- function(u, arms,ratio,
...
boundaries_4arm2stageORDNonInf_Zmod_SF <- function(theta,
                                                   noninf,
                                                   stage,
                                                   alpha, 
                                                   beta,
  ...
  first <- bounds_4arm2stageORDSF(u11, arms,r, 
  ...
  for (p in 1:length(p)){
    ...
    callf <- bounds_4arm2stageORDSF(seq(from = low, 
                                        to = up, by = -1/(10^p)),
                                    arms,
                                    r,
                                    alpha,
...
ratio <- r <- c(1,2) #allocation ratio sample size at different stages: n1=r[1], n2=r[2]*n1,...
...
fun <- boundaries_4arm2stageORDNonInf_Zmod_SF(theta = c(pc,pt1,pt2,pt3),
                                              noninf = noninf,
                                              stage = stage,
                                              alpha = alpha,
                                              beta = beta,
...
pop <- fun$`sample size per arm per stage`#seq(fun$`sample size per arm per stage`-20, fun$`sample size per arm per stage`-10, by = 1)

upper <- fun$a#1.2762#seq(fun$a-.2, fun$a+0.03, by = 0.01)
...
for(u in 1:length(pop)){
  ...
  for(n in 1:length(upper)){
    ...
    int <-  matrix(data=FALSE, nrow=nrow(scenario),
                   ncol=nsim)
    
    prop_ord <- list()
    
    prop_ord[[1]] <- matrix(data=NA, ncol=3,
                            nrow = nrow(scenario))
    ...
    estimatedssIA <-   matrix(data=NA, nrow=nrow(scenario),
                                          ncol=nsim)
    
    estimatedsamplesize <- matrix(data=NA, ncol=1,
                                  nrow = nrow(scenario))
    
    estimatedsamplesizeIA <- matrix(data=NA, ncol=1,
                                    nrow = nrow(scenario))
    ...
    durationtrialIA <- matrix(data=NA, ncol=1,
                              nrow = nrow(scenario))
    
    durIA <- matrix(data=0, nrow=nrow(scenario),
                    ncol=nsim)
    ...
    for (k in 1:nrow(scenario)){
      ...
      for (j in 1:nsim){
        ...
        # 
        # last_patandfu <- c(month_lastpatplac+durations[1]+6,
        #                       month_lastpatlong+durations[2]+6,
        #                       month_lastpatmedium+durations[3]+6,
        #                       month_lastpatshort+durations[4]+6)
        # 
        # overrecr_month <- max(last_patandfu-lastpatientsobserved)-min(last_patandfu-lastpatientsobserved)
        
        # continue to recruit until observe the patients on placebo
        
        count_over <- sample(c("C","L","M", "S"), ceiling((durations[1])*recrate),#+6)*recrate),#overrecr_month*recrate, #
                             prob = p, replace=TRUE)
        ...
        n_long_overc <- sum(count_over[1:ceiling((durations[1]-durations[2])*recrate)]=="L")
        n_medium_overc <- sum(count_over[1:ceiling((durations[1]-durations[3])*recrate)]=="M")
        n_short_overc <- sum(count_over[1:ceiling((durations[1]-durations[4])*recrate)]=="S")
        
        plac11st <- rbinom(n = n_plac, prob= mean0, size = 1)
        
        dur11st <- rbinom(n = n_long+n_long_overc, prob= mean1, size = 1)
        
        dur21st <- rbinom(n = n_medium+n_medium_overc, prob= mean2, size = 1)
        
        dur31st <- rbinom(n = n_short+n_short_overc, prob= mean3, size = 1)
        
        dur[k,j] <- months + durations[1]#+6
        
        durIA[k,j] <- months + durations[1]#+6
        
        estimatedss[k,j] <- n_plac+n_plac_over+
          n_long+n_long_over+
          n_medium+n_medium_over+
          n_short+n_short_over#length(plac11st)+length(dur11st)+length(dur21st)+length(dur31st)
        
        estimatedssIA[k,j] <- n_plac+n_plac_over+
          n_long+n_long_over+
          n_medium+n_medium_over+
          n_short+n_short_over
        ...
        if((z1 <= u1 &  z3 >=u1)||
          ...
          dur2st <- c(dur21st, rbinom(n = n_medium_over-n_medium_overc, prob= mean2, size = 1))
          
          dur3st <- c(dur31st, rbinom(n = n_short_over-n_short_overc, prob= mean3, size = 1))
          ...
          dur22 <- c(dur2st, rbinom(n = n_medium_2, prob= mean2, size = 1))
          
          dur23 <- c(dur3st, rbinom(n = n_short_2, prob= mean3, size = 1))
      ...
      estimatedsamplesizeIA[k] <- mean(estimatedssIA[k,])
      
      durationtrial[k] <- mean(dur[k,])
      
      durationtrialIA[k] <- mean(durIA[k,])
    ...
    colnames(durationtrialIA) <- "Expected duration (months) at IA"
    ...
    summary_ORD <- summary_ORD %>% mutate(
      ...
      `Expected duration (months) at IA`,
      stopsatIA,
      p,
      theta,
      typeI,
...
write.csv(summary_ORD, paste("4arm2stage_SF_corrected_ESSIA3_recrutrate", recrate, "_", Sys.Date(),
                             ".csv", sep = ""),
          row.names = F)

#write.csv(summary_ORD, "4arm2stage_SF_recrate20_6mFUatIA_03042023_recruitmonthly.csv")
