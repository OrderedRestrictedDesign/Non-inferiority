  ...
  for(n in 1:length(upper)){
    ...
    for (k in 1:nrow(scenario)){
      ...
      for (j in 1:nsim){
        ...
        count_over <- sample(c("C","L","M", "S"), ceiling((durations[1]-durations[4])*30.41), 
                             prob = p, replace=TRUE)
        ...
        n_medium_overc <- sum(count_over[1:ceiling((durations[1]-durations[3])*30.41)]=="M")
        n_short_overc <- sum(count_over[1:ceiling((durations[1]-durations[4])*30.41)]=="S")
        ...
        estimatedss[k,j] <- length(plac11st)+length(dur11st)+length(dur21st)+length(dur31st)
