...
rhonull <- c(1,1,1,1) #use balanced allocation ratio for critical bounds
...
####### RESTRUCTURE scenarios and simulation##############
...
########### Strategy 1: match sample sizes at the interim analysis ###########
  ...
  for(n in 1:length(upper)){
    ...
    for (k in 1:nrow(scenario)){
      ...
      for (j in 1:nsim){
        ...
        estimatedssIA[k,j] <- length(plac1st)+length(dur1st)+length(dur2st)+length(dur3st)
...
write.csv(summary_ORD, paste("4arm2stage_SI_corrected_ESSIA3_recrutrate", recrate,"_", Sys.Date(),
                             ".csv", sep = ""),
          row.names = F)

#write.csv(summary_ORD, "4arm2stage_SI_recrate20_6mFUatIA_03042023_recruitmonthly.csv")
