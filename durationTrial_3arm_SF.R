  ...
  for(n in 1:length(upper)){
    ...
    for (k in 1:nrow(scenario)){
      ...
      for (j in 1:nsim){
        ...
        count_over <- sample(c("C","L","S"), ceiling((durations[1])*recrate), #+6)*recrate), 
                             prob = p, replace=TRUE)
        ...
        n_short_overc <- sum(count_over[1:ceiling((durations[1]-durations[3])*recrate)]=="S")
        ...
        estimatedss[k,j] <- n_plac+n_plac_over+
          n_long+n_long_over+n_short+n_short_over#length(plac11st)+length(dur11st)+length(dur21st)
        
        estimatedssIA[k,j] <- n_plac+n_plac_over+
          n_long+n_long_over+n_short+n_short_over
    ...
    colnames(estimatedsamplesizeIA) <- "ESS Ordered ORD at IA"
    ...
    colnames(summary_ORD)[22:23]<- c("alloc ratio T_L/T_c",
                                     "alloc ratio T_S/T_c")
...
write.csv(summary_ORD, paste("3arm2stage_SF_corrected_ESSIA3_recrutrate", recrate, "_", Sys.Date(), 
                             ".csv", sep = ""),
          row.names = F)    
