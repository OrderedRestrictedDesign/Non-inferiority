...
popord <- 87#fun$`sample size per arm per stage`#seq(fun$`sample size per arm per stage`-15, fun$`sample size per arm per stage`, by = 1)
...
if(ushape == "triangular"){
  ...
  upp <- upper * (1 + ratio/max(ratio))/sqrt(ratio)
  
  
  low <- -upper * ((1 -3*ratio/max(ratio))/sqrt(ratio))[1]
}
...
write.csv(summary_ORD, paste("3arm2stageORD_TFPC_new3_recrutrate", recrate, "_", Sys.Date(), 
                             ".csv", sep = ""),
row.names = F)    
